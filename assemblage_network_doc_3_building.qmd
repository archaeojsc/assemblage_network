...
How do we do that? Well, one proven method is that we can test the fit of a linear regression between the the observed distribution of node degrees and an ideal power law probability distribution of those degrees. The higher the $R^2$ of the model fit, the closer the observed data is to a scale-free topology.

Basically we're doing a grid search. For each combination of similarity matrix and adjacency method,
