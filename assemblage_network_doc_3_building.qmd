...
Since our similarity scores are already in the range $[0,1]$, we don't really need quite the same sort transformation as we would if we needed to take any real number as an input. We just need to "bend" our linear input range to suppress some range of the smaller values. Luckily, there is a very simple way to do that by using an exponential function. We'll define a power adjacency[^4] function:
...
Increasing $\beta$ can be seen to push the smaller values towards zero, which increases the minimum value at which the adjacency score exceeds $0.5$ (thus, it's a *soft* threshold). At $\beta=2$, only similarity values of $s_{i,j} \geq 0.71$ would break the $0.5$ adjacency threshold and the weights of all lower values penalized. At $\beta=3$ it would take $s_{i,j} \geq 0.79$ and so on.
