...
Soft threshold methods are just a little more difficult to implement, consisting of a transformation of the raw similarity score that boost stronger similarities and suppresses the weaker ones. Those familiar with activation functions in deep learning will already be accustomed to the variety of approaches available. Soft thresholds result in weighted graphs, therefore do not reduce the graph's density or reduce degree distribution unless a subsequent (hard) threshold is applied as well.
