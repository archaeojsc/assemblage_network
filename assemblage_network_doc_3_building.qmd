...
Well, one proven method[^6] is that we can test the fit of a linear regression between the the observed distribution of network connectivity and an ideal power law probability distribution. The higher the $R^2$ of the model fit, the closer the observed data is to a scale-free topology. The meaning of "connectivity" is different for weighted (soft) and un-weighted (hard) adjacency, though. For un-weighted graphs, connectivity is simply the degree of each node. If the edges are weighted, though, connectivity is the sum of edge weights (or "strength") connected to each node.
