...
prov_sims %>% stack() %>% filter(values > 0) %>% 
  ggplot(aes(x = values)) +
...
# Plot densities by similarity measure
artifact_sims %>% stack() %>% filter(values > 0) %>% 
  ggplot(aes(x = values)) +
...
We can see similar patterns in which artifacts occur together, depending on the choice of similarity metric. If you remember from the last installment, the overlap coefficient captures subsets and has a more "permissive" approach to scoring similarities. Sørensen--Dice and Jaccard, however, penalize differences in their scoring, which lead to lower overall similarity scores
