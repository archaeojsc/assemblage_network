...
set.seed(8675309)
g <-
  graph_from_edgelist(matrix(sample(LETTERS[1:10], 50, replace = T), ncol = 2), directed = FALSE)
...
cliqueBP <-
  matrix(c(rep(
    paste0("cl", seq_along(cliques)), sapply(cliques, length)
  ), names(unlist(cliques))), ncol = 2,)
...
sets <-
  lapply(bp.adj.mis, function(x)
    cliqueBP[cliqueBP[, 1] %in% as_ids(x), 2])
